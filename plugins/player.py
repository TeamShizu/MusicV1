...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/TGStreamZone')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                InlineKeyboardButton('⚡️Only work on', url='https://t.me/TGStreamZone'),
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/TGStreamZoneChannel'),
            ]
            ]
        await m.reply("<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE](https://github.com/TeamShizu/KingdomMusic) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
